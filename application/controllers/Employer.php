            ...
            public function downloadcv($jobseeker_id = null)
            {
                if (!empty($jobseeker_id)) {
                    ...
                    if ($this->job_apply_model->check_resume_by_id($jobseeker_id, $company_id) == true) {
						...
						$data['js_personal_info'] = $this->job_seeker_personal_model->personalinfo_list_by_id($company_id);
						$data['job_seeker_photo'] = $this->Job_seeker_photo_model->photo_by_seeker($company_id);
						$data['city'] = $this->Master_model->getMaster('city',$where=false);
						$data['country'] = $this->Master_model->getMaster('country',$where=false);
						$data['state'] = $this->Master_model->getMaster('state',$where=false);
