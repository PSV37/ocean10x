            	...
            	<div class="row">
                    ...
                    <div class="col-md-4">
                      <?php $exp=($this->Job_seeker_experience_model->experience_list_by_id($v_applicant->job_seeker_id));?>
                        ...
                        <?=(!empty($exp[0]->company_Profile_id)?'<strong>'.$exp[0]->company_Profile_id.'</strong>':'');?></strong>
							         <!--<div>
                            <?=(!empty($exp[0]->company_Profile_id)?''.$exp[0]->designation.'('.$exp[0]->duration.')':'');?></strong>
                            </div>-->
                        </div>
                        <div class="exp">
                        	<?=(!empty($exp[1]->company_Profile_id)?'<strong>'.$exp[1]->company_Profile_id.'</strong>':'');?></strong>
                          <?=(!empty($exp[1]->company_Profile_id)?''.$exp[1]->designation.'('.$exp[1]->duration.')':'');?></strong>
                        </div>
                    </div>
